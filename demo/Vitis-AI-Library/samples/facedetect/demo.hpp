...
static int frame_nn = 0;
static float video_fps;
static string file_without_extension;
...
struct DecodeThread : public MyThread {
  ...
  void open_stream() {
    ...
    std::string base_filename = video_file_.substr(video_file_.find_last_of("/\\") + 1);
    std::string::size_type const p(base_filename. find_last_of('.'));
    file_without_extension = base_filename. substr(0, p);
    // LOG(INFO) << file_without_extension; //recent
    cv::VideoCapture camera(video_file_);
    video_fps = camera.get(CV_CAP_PROP_FPS);
...
struct DpuFilter : public Filter {
  ...
  cv::Mat run(cv::Mat& image) override {
    ...
    return processor_(image, result, false, frame_nn, video_fps, file_without_extension);
  }
...
int main_for_jpeg_demo(int argc, char* argv[],
  ...
  if (ENV_PARAM(SAMPLES_ENABLE_BATCH)) {
    ...
    for (auto i = 0u; i < results.size(); i++) {
      ...
      auto image = process_result(images[i], results[i], true, frame_nn, video_fps, file_without_extension);
    ...
    for (int i = start_pos; i < argc; ++i) {
      ...
      image = process_result(image, result, true, frame_nn, video_fps, file_without_extension);
      auto out_file =
